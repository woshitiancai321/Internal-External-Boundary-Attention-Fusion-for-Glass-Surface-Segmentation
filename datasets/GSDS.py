...
def make_dataset(quality, mode):
    ...
    logging.info(f'GSDS has a total of {len(all_items)} images in {mode} phase')

    logging.info(f'GSDS-{mode}: {len(all_items)} images')

    return all_items


class GSDSDateset(data.Dataset):
