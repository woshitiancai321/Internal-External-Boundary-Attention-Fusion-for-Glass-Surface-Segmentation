...
def make_dataset(quality, mode):
    ...
    logging.info(f'HSO has a total of {len(all_items)} images in {mode} phase')

    logging.info(f'HSO-{mode}: {len(all_items)} images')

    return all_items


class HSODateset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if((img.size[1] == mask.size[0])&(img.size[0] == mask.size[1])):
            mask = mask.T
        #mask.show()
